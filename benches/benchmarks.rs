...
pub fn arche_tape(c: &mut Criterion) {
    ...
    group.bench_function("static_frag_iter_20_entity", |b| {
        let mut bench = bench_static::frag_iter_20::Benchmark::new();
        b.iter(move || bench.run());
    });
    group.bench_function("dynamic_frag_iter_20_entity", |b| {
        let mut bench = bench_dynamic::frag_iter_20::Benchmark::new();
        b.iter(move || bench.run());
    });

    group.bench_function("static_frag_iter_200_entity", |b| {
        let mut bench = bench_static::frag_iter_200::Benchmark::new();
        b.iter(move || bench.run());
    });
    group.bench_function("dynamic_frag_iter_200_entity", |b| {
        let mut bench = bench_dynamic::frag_iter_200::Benchmark::new();
        b.iter(move || bench.run());
    });

    group.bench_function("static_frag_iter_2000_entity", |b| {
        let mut bench = bench_static::frag_iter_2000::Benchmark::new();
        b.iter(move || bench.run());
    });
    group.bench_function("dynamic_frag_iter_2000_entity", |b| {
        let mut bench = bench_dynamic::frag_iter_2000::Benchmark::new();
        b.iter(move || bench.run());
    });
